...
(function( $ ) { 'use strict';
	$('document').ready( function( $ ) {	
		...
		commentForm.submit(function(){
			...
			$( '#commentform .milky-way-comment-error' ).remove();
			$( '#commentform .milky-way-field-error' ).remove();
			var hasError = false;
			/**
			...
			$( '#commentform [aria-required=true]' ).each( function() {
				...
				if ( $.trim( $(this).val() ) == '' ) {
					...
					$(this).parent().append( ' <span class="milky-way-field-error" id="' + id + '">' + milky-wayComments.required + '</span>' );					hasError = true;
				}
				if ( $(this).attr( 'name' ) == 'email' && $.trim( $(this).val() ) != '' ) {
					var value = $(this).val();
					var valid = milky-wayValidateEmail( value );
					if ( !valid ) {
						$(this).attr( 'aria-describedby', id );
						$(this).parent().append( ' <span class="milky-way-field-error" id="' + id + '">' + milky-wayComments.emailInvalid + '</span>' );
						hasError = true;
					}
				}
			});
			 ...
			 */
			if ( hasError ) {
				statusdiv.html( '<p class="milky-way-comment-error">' + milky-wayComments.error + '</p>' ).focus();
				return false;
			}			
			 ...
			 */
			statusdiv.html( '<p class="milky-way-comment-processing">' + milky-wayComments.processing + '</p>' );
			...
			$.ajax({
				...
				error: function( XMLHttpRequest, textStatus, errorThrown ) { // [localize]
					statusdiv.html( '<p class="milky-way-comment-error">' + milky-wayComments.flood + '</p>' ).focus();
				},
				success: function( data, textStatus ){
					...
					if ( success ) {
						 ...
						 */						
						statusdiv.html('<p class="milky-way-comment-success" >'+status+'</p>').focus();
						//alert(data);
						 ...
						 */
						statusdiv.html('<p class="milky-way-comment-error" >'+status+'</p>').focus();
						commentForm.find('textarea[name=comment]').val('');
					}
				}
			} );
	 ...
	 * @return boolean 
	 */
	function milkyWayValidateEmail( value ) {
