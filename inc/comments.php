...
add_action( 'comment_post', 'milky_way_ajax_comments', 20, 2 );
/**
		...
		switch ( $comment_status ) {
			case '0':
				...
				$return = array( 
					'response' => '', 
					'success'  => 1, 
					'status'   => __( 'Your comment has been sent for moderation. It should be approved soon!', 'milky-way' ) 
				);
				wp_send_json( $return );
				break;
			case '1':
						...
						<li id="comment-' . $comment->comment_ID . '"' . $comment_class . ' tabindex="-1">
							<article id="div-comment-' . $comment->comment_ID . '" class="comment-body">
								...
								<div class="comment-author vcard">'.
									...
									.'<b class="fn">' . __( 'You said:', 'milky-way' ) . '</b> </div>
				...
				$return = array( 
					'response'=>$output, 
					'success' => 1, 
					'status'=> sprintf( __( 'Thanks for commenting! Your comment has been approved. <a href="%s">Read your comment</a>', 'milky-way' ), "#comment-$comment_ID" ) 
				);
				wp_send_json( $return );
				break;
			default:
				...
				$return = array( 
					'response' => '', 
					'success'  => 0, 
					'status'   => __( 'There was an error posting your comment. Try again later!', 'milky-way' ) 
				);
				wp_send_json( $return );
		}
	}
